...
def test_previsualizar_halcyon_respeta_la_ruta_configurada(monkeypatch, tmp_path):
    """H3: `previsualizar_halcyon`/`agregar_halcyon` ya no son un único
    `addInfo` -- las dos comparten `_localizar_y_leer_mpc`, que es donde
    vive esta llamada. No debe seguir usando la constante hardcodeada --
    confirma que de verdad llama a ruta_mpc_halcyon() en vez de la ruta de
    red fija."""
    monkeypatch.setenv("RADQA_HALCYON_MPC", str(tmp_path))
    capturada = {}
    ...
    halcyon_mod.previsualizar_halcyon(None, "2026-07-01")
