...
tabla `halcyon` (confirmado: `addInfo` es la única función que dispara el
...
@pytest.fixture
def preparar_addInfo(monkeypatch, tmp_path, carpeta_mpc_falsa):
...
class TestAddInfoAuditaLaIngesta:
    def test_createDB_exitoso_registra_en_audit_log(self, preparar_addInfo, monkeypatch):
    ...
    def test_fecha_ya_existente_no_duplica_ni_audita(self, preparar_addInfo, monkeypatch):
