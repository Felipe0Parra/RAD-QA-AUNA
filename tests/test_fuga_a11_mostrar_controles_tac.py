"""A11 (PLAN_FUGA_CONEXIONES_01-09.md §9, hallazgo del propio tripwire R5):
`load.py::mostrar_controles_tac` -- gemelo exacto de A9/A10 (Tomógrafo en
vez de Clinac ix/Halcyon). RIESGO ALTO: mismo `ALTER TABLE pruebas ADD
COLUMN mes_control` + `UPDATE pruebas SET mes_control = ...` sin ningún
`try`/`except`.

Mismo tratamiento que A9/A10: `with` acotado a la parte que usa `conn`;
P2 con `filtro_activo` reventando."""
import os
...
class TestA11MostrarControlesTacCierraSiempre:
    def test_filtro_activo_revienta_y_aun_asi_se_invoca_exit(
        ...
        with pytest.raises(RuntimeError, match="fallo inyectado"):
            load_mod.mostrar_controles_tac(None, QTableWidget(), "Físico de Prueba")
    ...
    def test_camino_normal_sigue_funcionando_igual(self, app, bd_temporal):
        ...
        con.execute(
            ...
            "VALUES (?, ?, ?, ?)",
            ("Tomógrafo", "Mensual", "01/2026", "Físico de Prueba"))
        con.commit()

        tabla = QTableWidget()
        load_mod.mostrar_controles_tac(None, tabla, "Físico de Prueba")
